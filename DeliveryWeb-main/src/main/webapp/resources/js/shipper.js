    ...
    fetch(endpoint).then(function (res) {
        ...
        if (d !== null) {
            ...
            for (let i = 0; i < data.length; i++)
                h += ` 
                <tr id="row${data[i].id}">
                    <td><img src="${data[i].avatarShipper}" width:"50"/></td>
                    <td></td>
                    <td>${data[i].licensePlate}</td>
                    <td>
                        <button class="btn btn-light">Sửa</button>
                        <div class="spinner-border text-secondary" style="display:none" id="load${data[i].id}"></div>
                        <button class="btn btn-danger" onclick="deleteShipper('${endpoint + "/" + data[i].id}', ${data[i].id}, this)">Xóa</button>
                    </td>
                </tr>
            `
            d.innerHTML = h;
